...
- ⚠️ Ne jamais fournir de **patch** ou diff : toujours les **fichiers complets modifiés**.  
- **Attention aux régressions** : tester avant envoi.  
- Lorsqu’un `.zip` est envoyé, **reprendre l’intégralité de son contenu** comme nouvelle base.  

## 🔹 Conventions de travail
- On ne travaille **que sur le dossier `dev/`**.  
- On ne travaille **que sur un fichier à la fois**, sauf demande explicite.  
- Chaque fonction doit comporter **un commentaire PHPDoc** (rôle, paramètres, retour).  
...
- **S100 – renommage de fonction** :  
  - `__mask_value` ➜ `maskValue` (camelCase).  
  - Tous les appels mis à jour.  
- **S1142 – trop de return** : `maskValue()` refactorée à **2 retours max (≤3)**.  
- **S3776 – complexité cognitive** : découpage en helpers (`normalizeKey()`, `looksLikeHttpUrl()`, etc.).  
- **php:S3358** appliqué.  
...
- Si une fonction est remplacée :  
  ...
  - ⚠️ Les appels existants doivent continuer à fonctionner.  
